...
The curvature formula. The gauge field is the curvature of the gauge potential. This principle is used to obtain the physical field strength from $A$.
...
The abelian reduction $F=dA$. In electromagnetism the gauge group is abelian, so $A \wedge A = 0$ and the field strength reduces to $dA$. An abelian group is a group whose elements commute. This principle is used to recover $\mathbf{B}=\nabla\times\mathbf{A}$ and $F_{\mu\nu}=\partial_{\mu}A_{\nu}-\partial_{\nu}A_{\mu}$.
...
The tensorial transformation of $F$. Under a gauge transformation the field strength transforms tensorially. This principle is used to keep $F$ physically meaningful while $A$ changes as a connection.

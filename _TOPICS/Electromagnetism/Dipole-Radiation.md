...
1\. Radiation is energy that leaves the source forever. That requires the outward energy flux, measured by the Poynting vector $\mathbf{S}$, to fall no faster than $1/r^{2}$ at large $r$. This principle is used to tell true radiating fields apart from near fields that keep energy near the charges.
...
3\. In that zone the electric and magnetic fields are transverse to the travel direction, perpendicular to each other, in phase, and related by $E/B = c$. Retarded time $t_{r} = t - r/c$ is the emission time at the source. This principle is used to find the fields that a distant receiver measures.
...
5\. The total average power of a sinusoidally oscillating dipole scales as $\omega^{4}$. Angular frequency $\omega$ is the oscillation rate in radians per second. This principle is used to explain why high-frequency sources radiate far more efficiently than low-frequency ones.
...
Note: These principles are the radiation criterion, the radiation-zone approximation, the radiation-zone field equations, the dipole radiation pattern, the $\omega^{4}$ dipole power law, the Larmor formula, and electric-dipole dominance in the multipole expansion.
